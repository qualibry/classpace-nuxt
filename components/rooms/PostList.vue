    ...
    >
      <div @click="redirectDetail(item)" class="block-nav">
        ...
        <div class="flex align-items-center text-sm">
          ...
          <span class="files ml-auto"
            >{{ item.attachments_count }} {{ $t("posts.filesAttached") }}</span
          >
        </div>
      </div>
      <template v-if="currentParticipation.can_manage_posts">
        ...
        <Button
          @click.prevent="deletePostHelper(item)"
          class="p-button-sm p-button-danger px-2 py-1 mr-2"
        >
