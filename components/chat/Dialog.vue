<template>
  <div class="flex flex-column dialog-container">
    <MessagesList ref="messageComponent" />
...
export default {
  ...
  created() {
    if (process.client) {
      ...
      const socket = new WebSocket(WS_URL + `api/v1/chat/?jwt_token=${token}&dialog_id=${dialogId}&limit=30`)
      socket.onmessage = (message) => {
        this.$store.commit('chat/PUSH_DIALOG_MESSAGE', JSON.parse(message.data))
        setTimeout(() => {
          this.$refs.messageComponent.$refs.messageWrapper.scrollTop = this.scrollFromTop
        }, 10)
      }
      this.socket = socket;
    }
  ...
  methods: {
    async sendMessage(e) {
      if (this.messageText !== '') {
        await this.socket.send(JSON.stringify({ text: this.messageText }))
        this.messageText = ""
      }
    },
    show(e) {
  ...
  computed: {
    ...mapGetters({
      scrollFromTop: 'chat/scrollFromTop'
    }),
  },
}
</script>
