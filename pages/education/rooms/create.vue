<template>
    <div class="rooms-create">
        ...
        <div class="xl:col-4 col-12">
            <div class="mb-3">
                <InputText :placeholder="$t('rooms.roomName')" v-model="form.name" class="block w-full" />
                <span class="text-sm text-red-400">{{ errors.name }}</span>
            </div>
            <Textarea :placeholder="$t('rooms.roomDescription')" v-model="form.description" :autoResize="true" rows="6"
        ...
        async createRoom() {
            ...
            if (Object.keys(this.errors).length == 0) {
                this.$toast.add({
                    ...
                    detail: `${this.$t('rooms.createRoomSuccess')}`,
                    life: 3000
                });
